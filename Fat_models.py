...
import sys
import os
# from quantumhybrid import Hybrid
# import qiskit
# import numpy as np
# why large kernel size is handling here??? 
...
class FASA(nn.Module):
    ...
    def refined_downsample(self, dim, window_size, kernel_size):
        ...
        for num in range(i):
            ...
            block.add_module('bn{}'.format(num), nn.BatchNorm2d(dim)) # SyncBatchNorm replaced by BatchNorm2d
            if num != i-1:
...
class FATBlock(nn.Module):
    ...
    def __init__(self, dim: int, out_dim: int, kernel_size: int, num_heads: int, window_size: int, 
        ...
        super().__init__()
        self.dim = dim
        # self.cpe = nn.Conv2d(dim, dim, kernel_size, 1, kernel_size//2, groups=dim) # By default: torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros', device=None, dtype=None)
        self.cpe = get_conv2d(dim, dim, kernel_size, 1, kernel_size//2, 1, dim, True) # get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.mlp_ratio = mlp_ratio
        self.norm1 = nn.GroupNorm(1, dim) # nn.GroupNorm(1, dim), it means each channel is treated as a separate group, effectively performing instance normalization.
    ...
    def forward(self, x: torch.Tensor):
        x = x + self.cpe(x)
        x = x + self.drop_path(self.attn(self.norm1(x)))   # Y = FASA(LN(X))+X ; attn is FASA, norm1 is LN
        x = self.downsample(x) + self.drop_path(self.ffn(self.norm2(x))) # Z= ConFFN(LN(Y)) + ShortCut(Y) here shorcut is downsampled path
        return x
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        ...
        self.conv = nn.Sequential(
                        ...
                        nn.BatchNorm2d(out_channels), #replaced
                        nn.ReLU()
                    )
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=96):
        super().__init__()
        self.conv1 = BasicBlock(in_channels, out_channels//2, 3, 2) # in_channel, out_channel, kernelsize, stride
...
class FAT(nn.Module):
    def __init__(self, in_chans, num_classes, embed_dims: List[int], depths: List[int], kernel_sizes: List[int],
        ...
        #
        self.patch_embed = PatchEmbedding(in_chans, embed_dims[0]) # in_channel, out_channel
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))] #.item extract scalar value from np.array or tensor, generate the values in the range of drop_path_rate. Total value defined by depth paths.
        self.layers = nn.ModuleList() # ModuleList() is a container that can be used to hold a list of PyTorch modules, and it ensures that the modules within it are properly registered as sub-modules.
        for i_layer in range(self.num_layers):
            if i_layer != self.num_layers-1:
                layer = FATlayer(depths[i_layer], embed_dims[i_layer], embed_dims[i_layer+1], kernel_sizes[i_layer],num_heads[i_layer],
                                    ...
                                    dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])], True, use_checkpoint) # downsampling True
                # what are the aurguments? --> Answer: depth: int, dim: int, out_dim: int, kernel_size: int, num_heads: int, 
                             # window_size: int, mlp_kernel_size: int, mlp_ratio: float, drop_paths=[0., 0.],
                             # downsample=True, use_checkpoint=False)
            else: 
                layer = FATlayer(depths[i_layer], embed_dims[i_layer], embed_dims[i_layer], kernel_sizes[i_layer],num_heads[i_layer],
                                    window_sizes[i_layer], mlp_kernel_sizes[i_layer], mlp_ratios[i_layer],  
                                    dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])], False, use_checkpoint) # Downsamping False
            self.layers.append(layer)
        self.norm = nn.GroupNorm(1, embed_dims[-1])
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.head = nn.Linear(embed_dims[-1], num_classes) if num_classes>0 else nn.Identity()
        # self.fc1 = nn.Linear(embed_dims[-1], 1)
        # self.hybrid = Hybrid(self.fc1.out_features, qiskit.Aer.get_backend('aer_simulator'), 100, np.pi / 2)
    ...
    def forward_feature(self, x):
        ...
        x: (b 3 h w)
        '''
        x = self.patch_embed(x)    # Conv Stem
        for layer in self.layers:
            x = layer(x)
        x = self.avgpool(self.norm(x)) # Avg pooling layer after last FAT block L4
        return x.flatten(1)
    ...
    def forward(self, x):
        ...
        # x = self.hybrid(x).T
        # x = torch.cat((x,1-x), -1)
        x= self.head(x) # # Output Layer --- comes on return place
        # x= self.sigmoid(x)
        return x# Output Layer
    
def build_model(in_chans, num_classes, embed_dims, depths,kernel_sizes, num_heads,window_sizes, mlp_kernel_sizes, mlp_ratios,drop_path_rate, use_checkpoint):
  ...
  model = FAT(
      ...
      mlp_kernel_sizes=mlp_kernel_sizes,
